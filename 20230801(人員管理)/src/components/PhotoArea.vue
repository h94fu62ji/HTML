...
<template>
    <div class="all-area">
        <div class="area h-100 relative overflow-hidden">
            ...
            <button type="button" class="w-10 h-10 rounded-full colorb opacity-50 hover:opacity-100 absolute top-44 left-2"
                @click="changeL">&lt;</button>
            <button type="button" class="w-10 h-10 rounded-full colorb opacity-50 hover:opacity-100 absolute top-44 right-2"
                @click="changeR">&gt;</button>
            <div class="absolute bottom-2 right-1/2 translate-x-1/2">
                <button type="button" class="w-10 h-2 rounded-full colorb opacity-50 hover:opacity-100 mx-1"
                    @click="change1"></button>
                <button type="button" class="w-10 h-2 rounded-full colorb opacity-50 hover:opacity-100 mx-1"
                    @click="change2"></button>
                <button type="button" class="w-10 h-2 rounded-full colorb opacity-50 hover:opacity-100 mx-1"
                    @click="change3"></button>
                <button type="button" class="w-10 h-2 rounded-full colorb opacity-50 hover:opacity-100 mx-1"
                    @click="change4"></button>
            </div>
        </div>
    </div>
</template>
